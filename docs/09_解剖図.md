...
| 3 | 大脳皮質 / 長期記憶 | `memory/retrieve.py` で思い出す。`memory/embed.py`（Ollama bge-m3）で意味の近さ。`db.run_maintenance` で忘れる | `memory_nodes`（episode 30日 / semantic 180日、pinned は消えない）、`memory_edges`、`memory_tags`、`memory_vectors` | 発言ごと / 4時間ごと |
...
| いつ | 人間 | ことは | 中身 |
|---|---|---|---|
| 会話中（覚醒） | ワーキングメモリと連想 | `messages` 直近12往復。発言ごとに `retrieve.py` が連想する | 保護記憶（pinned）は常に。タグ命中と意味の近さで関連を、続きのある話（open_topic）は別枠で。思い出した記憶は `[USED:]` で申告され、期限が延びる（使うほど残る） |
...
| 4時間ごと（新陳代謝） | 忘却 | `db.run_maintenance` | 期限を過ぎ、ピン留めでないものを消す。その前に必ず控えを取る。長く使われないタグの数えも戻す |

## 軽くした部分（人と違うところ）

シンプルさのために意図して省いたもの。設計の欠陥ではなく、知っておくと「人ならこうするのに」のずれが読めます。

- **想起は毎回の検索。** 人は「思い出せそうで出ない」があるが、ことはは発言ごとにタグと意味で引き直す。引っかからなければ、無いのと同じ。日記は想起の検索に乗らず、会話に見えるのは直近3日と習慣だけ
...
- **忘却は期限制。** 忘却曲線ではなく、日数と「使われたら延びる」の2つ。ピン留めは絶対に消えない
