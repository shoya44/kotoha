...
| 3 | 大脳皮質 / 長期記憶 | `memory/retrieve.py` で思い出す（強い記憶が先。日記も意味で引く）。`memory/embed.py`（Ollama bge-m3）で意味の近さ。`memory/strength.py` の強さが薄れ、床を切ると `db.run_maintenance` で消える | `memory_nodes`（strength、pinned は消えない）、`memory_edges`、`memory_tags`、`memory_vectors`、`diary_vectors` | 発言ごと / 4時間ごと |
| 4 | 扁桃体 / 情動 | 返事の `[MOOD:]`、時間帯の下地は `talk/figure.py`。黙っているあいだ半減期で薄れる（`chat.mood_intensity`） | `app_state.mood` / `mood_why` / `mood_at` | 返事のたび |
...
- **想起は毎回の検索。** 発言ごとにタグと意味で引き直す。「思い出せそうで出ない」は強さで作ってあり（薄れた記憶は強い手がかりが要る）、日記も意味で引ける（「先月どうだった？」にその頃の日記が添う）。後から不意に思い出す道はまだ無い
- **意識が続かない。** 往復ごとにプロンプトを組み直す。連続した「今」はなく、直近12往復の窓と、さっきまでの話題（`app_state.topic`）、覚えごとで繋いでいる。会話の外で「考え続ける」ことはない
- **感情は5段のラベル。** きっかけ1行と、黙っているあいだの薄れ（半減期）は持つが、快・不快と元気の2軸や、相手への信頼のような積み重ねはまだない
