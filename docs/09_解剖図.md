...
| 3 | 大脳皮質 / 長期記憶 | `memory/retrieve.py` で思い出す（強い記憶が先。日記も意味で引く。出なかったものは `jobs.maybe_afterthought` が後から「そういえば」）。`memory/embed.py`（Ollama bge-m3）で意味の近さ。`memory/strength.py` の強さが薄れ、床を切ると `db.run_maintenance` で消える | `memory_nodes`（strength、pinned は消えない）、`memory_edges`、`memory_tags`、`memory_vectors`、`diary_vectors` | 発言ごと / 4時間ごと |
