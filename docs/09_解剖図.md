...
違いは主に4つ。想起が毎回の検索であること、意識が連続しないこと、感情が2軸と5つのラベルであること、固着と再生がバッチ（空き時間と1日1回）であること。末尾の「軽くした部分」に書いてあります。
...
| 12 | 身体・表情・居場所 | `talk/figure.py`（立ち絵とふるまい。返事の `[FACE:]` で一瞬の表情）、`serve/hub.py`（実体はひとつ、器は複数）、`scripts/mascot.pyw` | `data/mascot.json` | 巡回の終わりと返事のたび |
...
| 14 | 内受容感覚（自分の身体の様子） | `talk/myself.py`。止まっていた長さ、起き直したら中身が違うこと、設定を変えられたこと。何が変わったかは分からない | `app_state.last_alive_at` / `self_*` | 起きたとき、設定を変えられたとき |
