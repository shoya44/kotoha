...
`reminders` と `app_state` は、ほかの表と繋がりません。
...
- `expires_at` を過ぎると消えます。**使われると延びます**（返事の `[USED:]` 起点）
...
- `done_at` は「言った」「取り消した」「古すぎて畳んだ」のいずれでも入ります
...
| `mood` / `mood_at` | いまの機嫌と、そうなった時刻（6時間で「ふつう」へ戻る） |
